...
# Load in data ================================================================
loaded_data = pyGTM.load_gtm_results(r"N:\PhD\GTM\upm10_stm\upm10_l4_80.h5")
...
lmax_I = int(loaded_data["metadata"]["user_settings"]["lmax_I"])
...
legend_labels = [f'$Y_{{{l},m}}^{{S_I}}$' for l in range(l_max + 1)]
...
plt.axhline(y=max_error, color='red', linestyle='--', linewidth=2, 
            alpha=0.8, label=f'Max Error: {max_error:.2e}')

plt.grid(True, linestyle='-', alpha=0.7)
plt.xlabel('Frequency (Hz)', fontsize=20)
plt.ylabel('Error metric, Equation (3)', fontsize=20)
plt.title('Error metric vs Frequency for individual interior pressure distributions, "Complex elastic structure"', fontsize=22, pad=15)
plt.ylim([1E-2,0.2])
...
order_power = np.zeros([lmax_O,len(frequencies)],dtype=np.complex128())

for id_l in range(lmax_I):
    print(id_l**2, id_l**2+2*id_l+1)
    
    order_power[id_l,:] += np.sum(GTM[id_l,id_l**2:id_l**2+2*id_l+1,:],axis=0)
    
order_power= order_power.T


# %%

aaa = np.sum(np.abs(synthesized_response_clm)**2,0)
bbb = np.sum(aaa,2)
ccc = np.sum(bbb,1)
